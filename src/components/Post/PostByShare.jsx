...
export default function PostByShare({ posts, post, isOwnProfile, onShare, user, controlActiveStatusPost, users }) {
  ...
  useEffect(() => {
    ...
    getOriginalPost();
  }, [post?.originalPostId]); // 👈 thêm vào đây

  // const [commentContent, setCommentContent] = useState("");

  const userId = user.id; // Giả định userId từ currentUser, thay bằng logic auth thực tế
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white shadow-md p-4 rounded-md text-gray-700 mb-3 flex flex-col cursor-pointer">
        {/* Header */}
        <div className="w-full h-10 flex items-center gap-2" onClick={() => {
        ...
        <div className="shadow rounded p-3">
          <div className="w-full h-10 flex items-center gap-2 cursor-pointer" onClick={() => {
            if (!originalPost?.userId?.id) return;
            navigate(`/profile/${originalPost.userId.id}`);
          }}>
            {originalPost?.userId?.profilePicture != null ? (
